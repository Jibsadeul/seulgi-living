...
- `RECEIPT` 결과는 영수증에서 구매일 또는 결제일을 찾아 `date`로 반환한다.
- `RECEIPT`에서 날짜를 읽을 수 없으면 `date=null`로 정규화한다.
- `INGREDIENT` 결과는 `date=null`로 정규화한다.
...
- 영수증 응답의 `date`는 ISO 8601 timestamp 문자열 또는 `null`이다.
- 식재료 응답의 `date`는 항상 `null`이다.
