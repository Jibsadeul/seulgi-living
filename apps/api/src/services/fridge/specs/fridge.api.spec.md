...
| 필드         | 타입   | 필수 | 설명           |
| ------------ | ------ | ---- | -------------- |
| ingredientId | string | ✓    | 냉장고 재료 ID |

### 요청 바디

모든 필드는 선택값이다. 단, 최소 1개 필드는 포함해야 한다.
...
- `getCurrentMemberId`로 현재 사용자 확인. 인증 실패 시 `401` throw
- 현재 사용자의 재료만 수정
- `ingredientId`에 해당하는 재료가 없거나 현재 사용자의 재료가 아니면 `404` 반환
- 요청 바디에 수정 필드가 하나도 없으면 `400` 반환
- `name`이 비어 있거나 50자 초과 시 `400` 반환
- `imageKey`가 비어 있거나 50자 초과 시 `400` 반환
- `quantity`가 1 미만이거나 999999 초과 시 `400` 반환
- `unit`이 비어 있거나 10자 초과 시 `400` 반환
...
- 유효한 요청 시 해당 재료가 수정되고 `204`를 반환한다
- `quantity`만 보내면 수량만 수정한다
- `name`, `imageKey`, `category`, `quantity`, `unit`을 함께 보내면 해당 필드를 한 번에 수정한다
- 요청 바디가 비어 있으면 `400`을 반환한다
- `ingredientId`에 해당하는 재료가 없으면 `404`를 반환한다
- 다른 사용자의 재료를 수정하려 하면 `404`를 반환한다
- 인증 실패 시 `401`을 반환한다
...
- 유효한 요청 시 해당 재료가 삭제되고 `204`를 반환한다
- `ingredientId`에 해당하는 재료가 없으면 `404`를 반환한다
- 다른 사용자의 재료를 삭제하려 하면 `404`를 반환한다
- 인증 실패 시 `401`을 반환한다
