...
### `parseRequiredDocuments` 분리 규칙 (POLICY-038)

`sbmsnDcmntCn` 원문은 자유 형식 텍스트라 실제 서류 목록(번호/글머리 기호로 줄마다 서류 하나, 또는 한 줄에 쉼표로 나열된 서류명)과, "서류 제출 불필요" 같은 안내 문장이 섞여 들어온다. 문장을 그대로 쉼표로만 분리하면 안내 문장 안의 쉼표(문장 연결용)까지 서류 항목으로 잘못 쪼개진다 (예: `"별도의 서류 제출은 불필요하며, 증빙 필요 시 서민금융진흥원에서 별도 안내 예정"` → 두 개의 가짜 서류로 분리됨).

- 원문에 줄바꿈(`\n`/`\r`)이 있으면 항상 줄 단위로 먼저 분리하고, 각 줄을 다시 쉼표/`、` 기준으로 분리한다 (기존 동작 유지).
- 줄바꿈이 없는 한 줄짜리 텍스트인 경우, 문장형 표현(`불필요`, `필요 시`, `예정`, `바랍니다`, `확인해주시기`, `해주세요`, `입니다`, `습니다`)이 포함되어 있으면 **쪼개지 않고 원문 전체를 하나의 항목으로 반환**한다 (안내 문장으로 판단).
- 문장형 표현이 없는 한 줄짜리 텍스트는 기존처럼 쉼표/`、` 기준으로 분리한다 (실제 서류 나열로 판단).
- 쉼표/`、` 분리는 **괄호(`()`/`（）`) 안의 구분자는 무시**한다(`splitOutsideParens`). 서류명에 부가 설명이 괄호로 붙고 그 안에 쉼표가 들어가는 경우가 흔함 (예: `"주민등록초본(신청월 발급분, 최근 5년간 주소 및 신고일 포함)"` → 괄호 안 쉼표까지 자르면 괄호 뒤쪽 설명이 별도 서류로 잘못 분리된다).
