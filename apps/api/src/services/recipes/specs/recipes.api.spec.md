...
- 각 레시피의 실제 스크랩 여부가 `isSaved`에 반영된다.
- 인증 실패 시 `401`을 반환한다.
...
- 해당 레시피의 실제 스크랩 여부가 `isSaved`에 반영된다.
- 인증 실패 시 `401`을 반환한다.
...
모든 타입에서 인증 필수. 토큰 없으면 `401`.

### 쿼리 파라미터

| 이름   | 타입                               | 필수 | 기본값 | 설명               |
| ------ | ---------------------------------- | ---- | ------ | ------------------ |
| `type` | `speed \| diet \| night \| fridge` | 필수 | —      | 추천 타입          |
| `page` | number                             | 선택 | 1      | 페이지 번호        |
| `size` | number                             | 선택 | 20     | 페이지당 레시피 수 |

### 타입별 필터 기준

| 타입     | 조건                                                       | 정렬                                                         |
| -------- | ---------------------------------------------------------- | ------------------------------------------------------------ |
| `speed`  | `recipe_steps` row 수 1 이상 3 이하                        | 단계 수 오름차순, 동률이면 스크랩 많은 순, 그 이후 `id DESC` |
| `diet`   | `calories ≤ 150` AND `protein ≥ 20` (`null`인 레시피 제외) | 칼로리 오름차순, 동률이면 스크랩 많은 순, 그 이후 `id DESC`  |
| `night`  | `calories ≥ 400` (`null`인 레시피 제외)                    | 최신순, 동률이면 스크랩 많은 순, 그 이후 `id DESC`           |
| `fridge` | 매칭률 > 0인 레시피만                                      | 매칭률 내림차순, 동률이면 스크랩 많은 순, 그 이후 `id DESC`  |

### fridge 매칭 상세

- 매칭률 = (냉장고 재료 중 레시피에 쓰인 것 수) / (냉장고 전체 재료 수)
- 레시피 전체 재료 수: `ingredients` JSONB의 모든 section별 `items` 원소 수 합계
- 일치 판정: 냉장고 재료 `name`이 레시피 재료 항목에 ILIKE `%name%`으로 하나라도 포함되면 매칭
- 냉장고에 같은 이름의 재료가 여러 개 있어도 1개로 취급한다 (이름 기준 중복 제거)
- 냉장고 재료가 없으면 빈 배열 반환 (`totalCount: 0`)

### 응답

`GET /api/recipes`의 `RecipeListResponse`와 동일한 형식

### 규칙

- 조회 대상은 `recipes` 테이블 전체 (source 무관)
...
- `type` 없이 요청하면 `400`을 반환한다.
- `type=speed`는 조리 단계가 3개 이하인 레시피만 반환한다.
- `type=diet`는 칼로리 ≤ 100이고 단백질 ≥ 25인 레시피만 반환한다. `calories`나 `protein`이 `null`인 레시피는 제외된다.
- `type=night`는 칼로리 ≥ 400인 레시피만 반환한다. `calories`가 `null`인 레시피는 제외된다.
- 비로그인 시 모든 타입에서 `401`을 반환한다.
- `type=fridge`이고 냉장고가 비어 있으면 빈 배열과 `totalCount: 0`을 반환한다.
- `type=fridge`는 매칭률 내림차순으로 정렬된 레시피를 반환한다.
- `isSaved`는 현재 사용자의 실제 스크랩 여부로 응답된다.
