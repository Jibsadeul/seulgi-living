...
- 화면에 진입하거나(포커스 복귀 포함) 다시 표시될 때마다 목록을 서버에서 다시 조회한다(`useFocusEffect` + `refetch`). 캐시된 데이터는 즉시 보여주고, 백그라운드에서 검증/갱신한다(stale-while-revalidate). 다른 화면에서 새로 스크랩한 정책은 캐시에 없는 항목이라 낙관적 업데이트로 끼워넣지 않으므로, 이 focus 재조회로 반영한다.
